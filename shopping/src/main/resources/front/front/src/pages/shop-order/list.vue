...
<div :style='{"width":"86%","padding":"20px","margin":"10px auto","position":"relative","background":"#fff"}'>
    ...
    <div class="section-title" :style='{"margin":"10px 0","fontSize":"24px","lineHeight":"54px","color":"#000","textAlign":"center","background":"#ffc054"}'>我的订单</div>
  ...
  export default {
    ...
    methods: {
      ...
      handleAction(index, row) {
        // 根据不同操作类型调用相应的后端接口
        if (index == '已支付') {
          // 使用pay方法处理支付，调用后端settle接口
          this.pay(row);
        } else if (index == '已退款') {
          // 使用refund方法处理退款，调用后端refund接口
          this.refund(row);
        } else {
          // 其他状态更新直接调用update接口
          row.status = index;
        ...
        refund(item) {
            this.$confirm(`${item.total>0?'确定退款吗？退款金额将返回账户中。':'是否退订？'}`, "提示", {
                ...
                // 调用后端退款接口
                this.$http.post(`orders/refund`, item).then(res => {
                    if(res.data.code == 0) {
                        // 刷新用户会话信息
                        let table = localStorage.getItem("UserTableName");
                        this.$http.get(table+'/session', {}).then(sessionRes => {
                            if(sessionRes.data.code == 0) {
                                this.user = sessionRes.data.data;
        ...
        pay(item) {
            ...
            this.$http.get(table+'/session', {}).then(res => {
                ...
                // 调用后端settle接口进行统一处理（更新订单状态、扣减库存、扣减余额）
                this.$http.post('orders/settle', orderData).then(res => {
                    if(res.data.code == 0) {
                        // 更新本地用户信息
                        this.$http.get(`${table}/session`).then(sessionRes => {
                            if(sessionRes.data.code == 0) {
			...
			this.$confirm(`是否取消订单？`, "提示", {
				...
				this.$http.post(`orders/update`, item).then(res => {
                    ...
                    this.$http.get(item.tablename+'/info/'+item.goodid, {}).then(res => {
                        ...
                        if(item.tablename == `yaopinxinxi`){
                            // 加回库存数量
                            good.alllimittimes = good.alllimittimes + item.buynumber;
                            this.$http.post(`yaopinxinxi/update`, good).then(res => {
                            });
                        }
						this.$message({
							message: '订单取消成功',
