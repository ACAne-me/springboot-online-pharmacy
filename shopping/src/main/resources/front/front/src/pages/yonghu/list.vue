	...
	<div class="list" :style='{"margin":"0 0 10px","minWidth":"calc(100% - 200px)","background":"#fff","flex":"1","order":"3"}'>
		...
		<div class="list1 index-pv1" :style='{"width":"100%","padding":"0 10px","background":"#fff","height":"auto"}'>
			<div :style='{"boxShadow":"0px 4px 5px 0px rgba(0,0,0,0.3)","padding":"10px","margin":"10px 1%","background":"#fff","display":"inline-block","width":"23%","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
				<img @click.stop="imgPreView(item.touxiang)" :style='{"width":"100%","objectFit":"cover","display":"block","height":"225px"}' v-if="item.touxiang && item.touxiang.substr(0,4)=='http'" :src="item.touxiang.split(',')[0]" class="image" />
				<img @click.stop="imgPreView(baseUrl + (item.touxiang?item.touxiang.split(',')[0]:''))" :style='{"width":"100%","objectFit":"cover","display":"block","height":"225px"}' v-else :src="baseUrl + (item.touxiang?item.touxiang.split(',')[0]:'')" class="image" />
				...
				<div :style='{"padding":"0 10px","lineHeight":"1.5","fontSize":"14px","color":"#333"}' class="name ">{{item.yonghuming}}</div>
