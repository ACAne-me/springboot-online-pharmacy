    ...
    methods: {
      ...
      chongzhi() {
        ...
        let rechargeData = {
          ...
          money: parseFloat(this.chongzhiForm.money),
          paymentType: this.chongzhiForm.radio,
          role: this.userTableName
        };
        
        // 使用addBalance接口进行充值
        this.$http.post('users/addBalance', rechargeData).then(res => {
          if (res.data.code == 0) {
            // 刷新用户会话信息
            this.setSession();
            this.$message({
