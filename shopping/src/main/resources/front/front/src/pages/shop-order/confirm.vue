<template>
  <div :style='{"width":"86%","padding":"20px","margin":"10px auto","position":"relative","background":"#fff"}'>
    <div class="section-title" :style='{"margin":"10px 0","fontSize":"24px","lineHeight":"54px","color":"#000","textAlign":"center","background":"#ffc054"}'>确认{{totalPrice>0?'下单':'预订'}}</div>
         ...
         payClick() {
            ...
            for(let index in this.tableData){
				...
				setTimeout(()=>{
					// 准备订单数据
					let order = {
						orderid: String(this.createOrder()),
						...
						remark: this.remark,
						status: '已支付', // settle接口会处理支付逻辑
						sfsh: '',
						role: this.userTableName,
					};
					
					// 调用后端settle接口进行统一处理（创建订单、扣减库存、扣减余额）
					this.$http.post('orders/settle', order).then(res => {
						if(res.data.code == 0) {
							// 更新本地用户信息
							this.$http.get(`${this.userTableName}/session`).then(sessionRes => {
							...
							// 删除购物车数据(如果是购物车下单)
							if (item.id) {
								this.$http.post('cart/delete', [item.id]).then(() => {});
							}
							...
							this.$message({
								message: res.data.msg || '购买失败',
								type: 'error',
								duration: 1500,
								onClose(){
									if(res.data.msg && res.data.msg.includes('余额不足')) {
