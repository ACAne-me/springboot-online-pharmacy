					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('orders'+'/'+orderStatus,'物流')&&scope.row.id" type="success" @click="logisticsUpdate(scope.row.id)">
								<span class="icon iconfont icon-wuliu14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								物流
							</el-button>
							<el-button class="btn8" v-if="isAuth('orders'+'/'+orderStatus,'发货')&&scope.row.id" type="success" @click="updateHandler(scope.row)">
								<span class="icon iconfont icon-fahuo2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								发货
							</el-button>
							<el-button class="btn8" v-if="isAuth('orders'+'/'+orderStatus,'确认收货')&&scope.row.id" type="success" @click="updateHandler2(scope.row)">
								<span class="icon iconfont icon-dagou" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								确认收货
							</el-button>
	...
	export default {
		data() {
			return {
				...
				returnShForm: {},
				returnShRules:{
					sfsh:[{ required: true, message: '审核状态不能为空', trigger: ['blur', 'change'] },],
					shhf:[{ required: true, message: '审核回复不能为空', trigger: ['blur', 'change'] },],
				},
			...
			returnShSave(){ 
				...
				this.$refs["returnShForm"].validate(valid => {
					if(valid){
							...
							this.$http({
								...
								if(res.data&&res.data.code==0){
									...
									this.$http({
										...
										if(resUpdate.data&&resUpdate.data.code==0){
											this.$message({
												message: "审核成功，退款处理完成",
												type: "success",
												duration: 1500,
									...
									this.$message({
										message: "退款处理失败: " + (res.data.msg || '未知错误'),
										type: "error",
										duration: 1500
									})
								}
						...
						this.$message({message: '请填写所有必填项并确保数据正确', type: 'warning'});
						return false;
					}
				})
			},
