...
<div class="content" :style='{"minHeight":"calc(100vh - 70px)","padding":"0px 30px 20px","overflow":"hidden","alignItems":"flex-start","flexWrap":"wrap","background":"url(http://codegen.caihongy.cn/20231107/94f950da136d4785ad6630571b544668.png) no-repeat center top / cover","display":"flex","fontSize":"14px","justifyContent":"space-between","height":"100%"}'>
	...
	<div :style='{"border":"0px solid rgba(126, 96, 16, .1)","padding":"0px 0","margin":"0 0 10px","alignItems":"center","color":"#1e3c4f","display":"flex","float":"left","justifyContent":"center","flexWrap":"wrap","background":"none","width":"49%","fontSize":"14px","order":"0"}'>
		...
		<div :style='{"boxShadow":"0 0px 0px rgba(0,0,0,.3)","padding":"30px 20px","margin":"0 4% 20px 0","borderColor":"rgba(126, 96, 16, .1)","alignItems":"center","textAlign":"left","display":"flex","justifyContent":"space-between","borderRadius":"8px","background":"#fff","borderWidth":"0 0 0px","width":"48%","borderStyle":"solid"}' v-if="isAuth('yaopinxinxi','首页总数')">
			...
			<div :style='{"width":"auto","flexDirection":"row","justifyContent":"center","display":"flex"}'>
				<div :style='{"margin":"5px 20px 0 0","lineHeight":"24px","fontSize":"24px","color":"inherit","fontWeight":"bold","height":"24px"}'>{{yaopinxinxiCount}}</div>
				<div :style='{"margin":"5px 0","lineHeight":"24px","fontSize":"inherit","color":"inherit","height":"24px"}'>药品信息总数</div>
			</div>
		</div>
	</div>
	<!-- statis -->
	...
	<div class="type2" :style='{"alignContent":"flex-start","padding":"0","margin":"0","borderRadius":"8px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","position":"relative","justifyContent":"space-between","height":"auto","order":"10"}'>
		<div id="yaopinxinxiChart1" class="echarts1" v-if="isAuth('yaopinxinxi','首页统计')"></div>
		<div id="yaopinxinxiChart2" class="echarts2" v-if="isAuth('yaopinxinxi','首页统计')"></div>
	</div>
</div>
</template>
<script>
...
export default {
	...
	mounted(){
		this.init();
		this.getyonghuCount();
		this.getyaopinxinxiCount();
		this.yaopinxinxiChat1();
		this.yaopinxinxiChat2();
	},
	methods:{
		// 词云
		...
		getyaopinxinxiCount() {
			this.$http({
				url: `yaopinxinxi/count`,
				method: "get"
			}).then(({
				data
			}) => {
				if (data && data.code == 0) {
					this.yaopinxinxiCount = data.data
				}
			})
		},
		yaopinxinxiChat1() {
        ...
        var yaopinxinxiChart1 = echarts.init(document.getElementById("yaopinxinxiChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                yaopinxinxiChart1.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    yaopinxinxiChart1.resize();
                };
            }
        });
      })
    ...
    yaopinxinxiChat2() {
      this.$nextTick(()=>{
        ...
        var yaopinxinxiChart2 = echarts.init(document.getElementById("yaopinxinxiChart2"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                yaopinxinxiChart2.setOption(option);
				
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    yaopinxinxiChart2.resize();
                };
            }
        });
      })
