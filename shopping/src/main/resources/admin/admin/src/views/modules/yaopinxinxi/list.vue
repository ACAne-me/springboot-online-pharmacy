<template>
	<div class="main-content" :style='{"padding":"0px 30px 30px","fontSize":"14px","color":"#666","background":"none"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"flex-end"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"0px","margin":"0 10px 20px","borderRadius":"0 0 8px 8px","textAlign":"right","background":"none","display":"block","width":"auto","fontSize":"inherit","order":"1"}' >
					...
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}' class="select" label="药品分类" prop="yaopinfenlei">
						...
						<el-select clearable v-model="searchForm.yaopinfenlei" placeholder="请选择药品分类" >
							<el-option v-for="(item,index) in yaopinfenleiOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">药品规格</label>
						<el-input v-model="searchForm.yaopinguige" placeholder="药品规格" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","display":"inline-block"}'>
				...
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#fff","flexWrap":"wrap","textAlign":"right","flexDirection":"row","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"flex-end","order":"2"}'>
					...
					<el-button class="btn18" v-if="isAuth('yaopinxinxi','药品库存统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						药品库存统计
					</el-button>
					<el-button class="btn18" v-if="isAuth('yaopinxinxi','药品分类统计')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						药品分类统计
					</el-button>
				</el-row>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('yaopinxinxi','药品入库')" @click="rukuxinxiCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
							...
							<el-button class="view" v-if="isAuth('yaopinxinxi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="yaopinfenleiChart2" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
import rukuxinxiCrossAddOrUpdate from "../rukuxinxi/add-or-update";
	export default {
		...
		methods: {
			...
			rukuxinxiCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        ...
        this.$http({
            url: `yaopinxinxi/value/yaopinmingcheng/alllimittimes`,
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        var yaopinfenleiChart2 = echarts.init(document.getElementById("yaopinfenleiChart2"),'macarons');
        this.$http({
            url: "yaopinxinxi/group/yaopinfenlei",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                let pArray = []
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].yaopinfenlei);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].yaopinfenlei
                    })
                }
                var option = {};
                option = {
                ...
                // 使用刚指定的配置项和数据显示图表。
                yaopinfenleiChart2.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    yaopinfenleiChart2.resize();
                };
            }
        });
      })
    ...
    init () {
          ...
          let urlalllimittimes = 'yaopinxinxi/remind/alllimittimes/1';
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.yaopinguige!='' && this.searchForm.yaopinguige!=undefined){
            params['yaopinguige'] = '%' + this.searchForm.yaopinguige + '%'
          }
	...
	this.$router.push({path:'/discussyaopinxinxi',query:{refid:id}});
    },
    // 下载
    download(file){
