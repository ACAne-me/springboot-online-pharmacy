  ...
  <div :style='{"width":"100%","padding":"20px 10%","margin":"30px auto","position":"relative"}'>
    ...
    <div class="buy" v-if="seat!=1 && tableData.length > 0">
      <template v-if="type == 2">
        <span style="color: blue;margin-right: 10px;font-weight: 700;">{{totalPrice}} 积分</span>
        <el-button type="warning" @click="jfClick">兑换</el-button>
      </template>
      <template v-else background-color: blue>
        <el-button type="success" @click="payClick" :style="{ backgroundColor: 'red', borderColor: 'red',fontSize: '18px', padding: '10px 20px', width: '150px', height: '50px' }" >支付 ￥{{totalPrice}}</el-button>
      </template>
    </div>
    <div class="buy" v-if="seat==1 && tableData.length > 0">
      <template v-if="type == 2">
      ...
      <template v-else>
        <span style="color: blue;margin-right: 10px;font-weight: 700;" v-if="totalPrice>0"><span :style='{"fontSize":"20px"}'>￥</span>{{totalPrice}}</span>
