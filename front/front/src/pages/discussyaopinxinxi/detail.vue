		...
		<div class="attr" :style='{"width":"calc(100% - 500px)","margin":"0 0 0 30px","position":"relative","background":"#fff","display":"flex","order":"2"}'>
			...
			<div class="info" :style='{"width":"100%"}'>
				...
				<div class="btn" :style='{"row-gap":"20px","width":"470px","padding":"10px 0","flexWrap":"wrap","display":"block"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 10px 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#598969","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('discussyaopinxinxi','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 10px","margin":"0 5px 10px 0","color":"#fff","borderRadius":"4px","background":"#00AA3A","display":"inline-block","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' v-if="btnAuth('discussyaopinxinxi','删除')" @click="delClick">删除</el-button>
  ...
  export default {
    ...
    methods: {
	  ...
	  addDiscussNum(type){
		...
		// 删除
		async delClick(){
			await this.$confirm('是否删除此药品信息评论表？')
			  .then(_ => {
			    this.$http.post('discussyaopinxinxi/delete', [this.detail.id]).then(async res => {
					if (res.data.code == 0) {
						this.$message({
							type: 'success',
							message: '删除成功!',
