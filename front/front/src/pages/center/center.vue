...
<div class="center-preview" :style='{"width":"80%","margin":"30px auto","position":"relative"}'>
	<div class="title" :style='{"margin":"20px 0","color":"#333","textAlign":"center","display":"block","width":"100%","fontSize":"20px","lineHeight":"54px"}'>{{ title }}</div>
	
	<div class="info" :style='{"padding":"20px","grid-area":"1/1/2/2","display":"none","height":"auto"}'>
    ...
    <el-tabs tab-position="left" :style='{"width":"100%","margin":"20px 0 0","flexWrap":"wrap","display":"flex"}' @tab-click="handleClick">
      <el-tab-pane label="个人中心">
        <el-form class="center-preview-pv" ref="sessionForm" :model="sessionForm" :rules="rules" label-width="80px">
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' v-if="userTableName=='yonghu'" label="用户名" prop="yonghuming">
          ...
          <el-form-item v-if="userTableName == 'yonghu'" label="头像" prop="touxiang"  label-width="100px" style="padding:10px;margin:0 0 10px;float:left ;display: inline-block;" >
			 <file-upload style="margin: 40px;" tip="点击上传头像" action="file/upload" :limit="1" :multiple="true" :fileUrls="sessionForm.touxiang ? sessionForm.touxiang : ''" @change="yonghutouxiangHandleAvatarSuccess" >
			</file-upload> 
		</el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' v-if="userTableName=='yonghu'" label="年龄" prop="nianling">
          ...
          <el-form-item class="balance" :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' v-if="userTableName=='yonghu'" label="余额">
            <div :style='{"flexWrap":"wrap","display":"flex"}'>
				<el-input v-model="sessionForm.money" placeholder="余额" readonly></el-input>
				<div @click="dialogFormVisibleMoney = true" :style='{"border":"0","cursor":"pointer","padding":"0 15px","margin":"0 0px 0 0","color":"rgba(255, 255, 255, 1)","display":"inline-block","outline":"none","borderRadius":"0px","background":"#00AA3A","width":"90px","lineHeight":"40px","fontSize":"14px","height":"40px"}'>点我充值</div>
			</div>
		  </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' v-if="userTableName=='yonghu'" label="密保问题" prop="pquestion">
            <el-input v-model="sessionForm.pquestion" placeholder="密保问题" ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px","float":"left","display":"inline-block"}' v-if="userTableName=='yonghu'" label="密保答案" prop="panswer">
