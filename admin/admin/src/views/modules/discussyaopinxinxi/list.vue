	...
	<div class="main-content" :style='{"padding":"0px 30px 30px","fontSize":"14px","color":"#666","background":"none"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"0px 0px 0","margin":"0px","overflow":"hidden","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"flex-end"}' :inline="true" :model="searchForm">
				...
				<el-row :style='{"padding":"0px","margin":"0 10px 20px","borderRadius":"0 0 8px 8px","textAlign":"right","background":"none","display":"block","width":"auto","fontSize":"inherit","order":"1"}'>
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 20px","outline":"none","color":"#fff","borderRadius":"30px","background":"#ffc054","width":"auto","fontSize":"inherit","transition":"all 0.3s","height":"38px"}' type="success" @click="backClick()">返回</el-button>
				</el-row>
				<el-row class="actions" :style='{"margin":"0px 0 20px","color":"#fff","flexWrap":"wrap","textAlign":"right","flexDirection":"row","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"flex-end","order":"2"}'>
					<el-button class="add" v-if="isAuth('discussyaopinxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia17" :style='{"color":"inherit","margin":"0 2px","fontSize":"inherit"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('discussyaopinxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"border":"0px solid #eee","width":"100%","padding":"0","margin":"0px 0 0","borderRadius":"20px","background":"rgba(255,255,255,1)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('discussyaopinxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai10" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"none","height":"40px"}'></span>
								修改
							</el-button>

							<el-button class="view" v-if="isAuth('discussyaopinxinxi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
...
import AddOrUpdate from "./add-or-update";
	...
	this.$router.push({path:'/discussdiscussyaopinxinxi',query:{refid:id}});
    },
	backClick(){
		history.back()
	},
	...
	discussyaopinxinxistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'discussyaopinxinxi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
    ...
    async deleteHandler(id ,refid ) {
		...
		await this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
			...
			await this.$http({
				url: "discussyaopinxinxi/delete",
				method: "post",
				data: ids
			}).then(async ({ data }) => {
				if (data && data.code === 0) {
					this.$http({
						url: 'yaopinxinxi/info/' + refid,
						method: "get",
					}).then(res=>{
